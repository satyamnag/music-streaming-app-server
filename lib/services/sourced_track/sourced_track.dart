...
import 'package:sangeet/models/playback/track_sources.dart';
...
import 'package:sangeet/services/sourced_track/exceptions.dart';
...
class SourcedTrack extends BasicSourcedTrack {
  ...
  static Future<SourcedTrack> fetchFromTrack({
    required SangeetFullTrackObject query,
    required Ref ref,
  }) async {
  ...
  static List<SangeetAudioSourceMatchObject> rankResults(
    List<SangeetAudioSourceMatchObject> results,
  ...
  static Future<List<SangeetAudioSourceMatchObject>> fetchSiblings({
    required SangeetFullTrackObject query,
    required Ref ref,
  }) async {
    final audioSource = await ref.read(audioSourcePluginProvider.future);
    ...
    final videoResults = <SangeetAudioSourceMatchObject>[];
  ...
  Future<SourcedTrack> refreshStream() async {
    ...
    List<SangeetAudioSourceStreamObject> validStreams = [];
  ...
  SangeetAudioSourceStreamObject? getStreamOfQuality(
    SangeetAudioSourceContainerPreset preset,
    int qualityIndex,
  ) {
    ...
    final exactMatch = sources.firstWhereOrNull(
      (source) {
        ...
        if (quality case SangeetAudioLosslessContainerQuality()) {
          return source.sampleRate == quality.sampleRate &&
              source.bitDepth == quality.bitDepth;
        } else {
          return source.bitrate ==
              (quality as SangeetAudioLossyContainerQuality).bitrate;
        }
      },
    );
  ...
  SangeetAudioSourceContainerPreset? get qualityPreset {
